...
void BulletManager::createPlayerBullet(float posPlayerx, float posPlayery, int direction, bool spreadGun, ShaderProgram &shaderProgram) {
	...
	new_bull->createBullet(posPlayerx, posPlayery, direction, spreadGun, NO_DEVIATION, 1, shaderProgram, 1.5f);
	activeBullets.emplace_back(new_bull);
	if (spreadGun) {
		new_bull = new Bullet;
		new_bull->setTileMap(map);
		new_bull->createBullet(posPlayerx, posPlayery, direction, spreadGun, DEVIATION_1L, 1, shaderProgram, 1.5f);
		activeBullets.emplace_back(new_bull);

		new_bull = new Bullet;
		new_bull->setTileMap(map);
		new_bull->createBullet(posPlayerx, posPlayery, direction, spreadGun, DEVIATION_2L, 1, shaderProgram, 1.5f);
		activeBullets.emplace_back(new_bull);

		new_bull = new Bullet;
		new_bull->setTileMap(map);
		new_bull->createBullet(posPlayerx, posPlayery, direction, spreadGun, DEVIATION_1R, 1, shaderProgram, 1.5f);
		activeBullets.emplace_back(new_bull);

		new_bull = new Bullet;
		new_bull->setTileMap(map);
		new_bull->createBullet(posPlayerx, posPlayery, direction, spreadGun, DEVIATION_2R, 1, shaderProgram, 1.5f);
		activeBullets.emplace_back(new_bull);
	}
}
