	...
	for(int j=0; j<mapSize.y; j++)
	{
		for(int i=0; i<mapSize.x; i++)
		{
			...
			if(tile == " ")
				...
				map[j*mapSize.x+i] = (tile[0] - int('0')) * 10 + tile[1] - int('0');
...
bool TileMap::collisionMoveDown(const glm::vec2 &pos, const glm::ivec2 &size, float *posY) const
{
	...
	for(int x=x0; x<=x1; x++)
	{
		if(map[y*mapSize.x+x] == 9 || map[y*mapSize.x + x] == 12 || map[y*mapSize.x + x] == 32 || map[y*mapSize.x + x] == 33 || map[y*mapSize.x + x] == 34)
		{
